...
@property (nonatomic, copy) kImageSizeRequestCompleted sizeRequestCompletion;
@end

@implementation NSURL (RemoteSize)

- (void)setSizeRequestCompletion: (kImageSizeRequestCompleted) block {
    objc_setAssociatedObject(self, &kSizeRequestCompletionKey, block, OBJC_ASSOCIATION_COPY);
}

- (kImageSizeRequestCompleted)sizeRequestCompletion {
...
- (void)connection:(NSURLConnection*)connection didFailWithError:(NSError *)error {
    if(self.sizeRequestCompletion) self.sizeRequestCompletion(self, CGSizeZero);
}
...
@implementation UIImage (KJRemoteSize)
